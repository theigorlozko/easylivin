...
export default function ManagerProfilePage() {
  ...
  const managerId = params?.cognitoId as string;
  const { data: manager, isLoading, isError } = useGetManagersQuery(managerId);

  if (isLoading)
    return <p className="p-4 text-muted-foreground">Loading manager details...</p>;
  if (isError || !manager)
    return <p className="p-4 text-destructive">Failed to load manager profile.</p>;
